#ifndef _CONFIG_H_
#define _CONFIG_H_

#define GL_GLEXT_PROTOTYPES

#define WIDTH_SCREEN 640
#define HEIGHT_SCREEN 640

#define BORDE_IZQUIERDO 1<<0
#define BORDE_SUPERIOR  1<<1
#define BORDE_DERECHO   1<<2
#define BORDE_INFERIOR  1<<3

#define FPS_LIMIT 30.0f
#define FPS_DELAY 1000.0f / FPS_LIMIT

/***NUM SPRITES***/
#define MODULES_BALAS 1
#define MODULES_TITULOS 1
#define MODULES_ENEMIGO 1
#define MODULES_GANASTE 1
#define MODULES_JUGANDO 1
#define MODULES_MENU 1
#define MODULES_PERDISTE 1
#define MODULES_MINAVE 1

/***SPRITES***/
#define MODULO_MINAVE_NAVE 0
#define MODULO_MENU_FONDO 1
#define MODULO_ENEMIGO_NAVE 2
#define MODULO_TEXTO_TITULO 3
#define MODULO_TEXTO_NOMBRE 4
#define MODULO_TEXTO_MENU_OPCION1 5
#define MODULO_TEXTO_MENU_OPCION2 6
#define MODULO_TEXTO_MENU_OPCION1_SELECCIONADO 7
#define MODULO_TEXTO_MENU_OPCION2_SELECCIONADO 8
#define MODULO_BALAS_BALA 9

#define MAXIMO_DE_BALAS 30
#define MAXIMO_DE_NIVELES 5
#define MAXIMO_DE_ENEMIGOS 15

#define NAVE_PROPIA 0
#define NAVE_ENEMIGA 1

#define CERO 0
#define UNO 1


#endif